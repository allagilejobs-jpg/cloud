...
const STORAGE_KEY = "p6-construction-scheduler-v1";
const THEME_KEY = "p6-construction-theme";
...
function App() {
  ...
  const exportProgress = () => {
    const data = { progress, exportedAt: new Date().toISOString(), app: "p6-construction-scheduler-v1" };
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `p6-construction-progress-${new Date().toISOString().split("T")[0]}.json`;
    a.click();
  ...
  if (!loaded) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg, #0a0e1a)", color: "#14B8A6", fontFamily: "monospace" }}>
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)", fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      <style>{`
        ...
        :root, [data-theme="dark"] {
          ...
          --primary: #14B8A6; --primary-light: #14B8A620;
        }
        [data-theme="light"] {
          ...
          <div className="no-print" style={{ display: "flex", gap: 5, flexWrap: "wrap", alignItems: "center" }}>
            ...
            {[["dashboard", "📅 Dashboard"], ["projects", "🚀 Projects"], ["resources", "📚 Resources"], ["gaps", "🔥 Gaps"], ["certs", "🏅 Certs"]].map(([v, l]) => (
              <button 
                ...
                style={{ padding: "6px 12px", background: view === v ? "#14B8A6" : "var(--card-bg)", color: view === v ? "#042f2e" : "var(--text-secondary)", fontSize: 12, border: view === v ? "none" : "1px solid var(--border)" }}
              >
                {l}
              </button>
            ))}
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "20px 20px 60px" }}>
        ...
        {view === "dashboard" && !sel && (
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10, marginBottom: 16 }}>
              {[
                { l: "Progress", v: `${pct}%`, c: pct === 100 ? "#10b981" : "#14B8A6", bar: true },
                ...
                <div key={i} className="cd" style={{ padding: 14, textAlign: "center" }}>
                  ...
                  {s.bar && (
                    <div style={{ marginTop: 6 }}>
                      <ProgressBar current={cLabs + cProj} total={tLabs + tProj} color="#14B8A6" />
                    </div>
                  )}
                </div>
              ))}
            ...
            <div className="cd" style={{ padding: 16, marginBottom: 16, borderLeft: "3px solid #B7791F" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
                ...
                <span style={{ fontSize: 12, color: "var(--text-muted)" }}>
                  Primavera P6 construction scheduling focus
                </span>
              </div>
            ...
            <div className="no-print" style={{ display: "flex", gap: 4, flexWrap: "wrap", marginBottom: 16 }}>
              {[
                ...
                <button
                  ...
                  className="chip"
                  style={{
                    background: filter === f.key ? (CERT_COLORS[f.key] || "#14B8A6") + "15" : "transparent",
                    borderColor: filter === f.key ? (CERT_COLORS[f.key] || "#14B8A6") + "50" : "var(--border)",
                    color: filter === f.key ? (CERT_COLORS[f.key] || "#14B8A6") : "var(--text-muted)"
                  }}
                >
                  {f.label}
                </button>
              ...
              <div style={{ display: "grid", gap: 8 }}>
                {RESOURCES.map((r, i) => (
                  ...
                  >
                    ...
                    <span style={{ color: "#14B8A6", fontWeight: 500 }}>{r.text}</span>
                  </a>
                ))}
              </div>
            </div>
            
            <div style={{ background: "#14B8A608", border: "1px solid #14B8A620", borderRadius: 12, padding: 16 }}>
              <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 10 }}>💡 How to Use Claude as Your Scheduling Coach</div>
        ...
        {view === "gaps" && (
          <div style={{ animation: "fadeUp .4s ease" }}>
            ...
            {[
              { label: "CRITICAL - Would hurt delivery or interviews", color: "#ef4444", items: GAP_INFUSIONS.filter(g => g.severity === "critical") },
              { label: "IMPORTANT - Makes the schedule defensible", color: "#F59E0B", items: GAP_INFUSIONS.filter(g => g.severity === "important") },
              ...
              {CERTS.map((c, i) => (
                <div key={i} className="cd" style={{ padding: 16, display: "flex", alignItems: "center", gap: 14 }}>
                  ...
                  <div style={{ 
                    ...
                    background: (CERT_COLORS[c.code] || "#14B8A6") + "15",
                    color: CERT_COLORS[c.code] || "#14B8A6",
