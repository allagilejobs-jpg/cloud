...
export default function WeekDetail({ week, progress, onToggle, onBack, onNavigate, totalWeeks, copiedId, onCopy }) {
  ...
  const [notes, setNotes] = useState(() => {
    try {
      const saved = localStorage.getItem(`project-scheduling-notes-${week.week}`);
  ...
  const saveNote = (id, text) => {
    ...
    setNotes(updated);
    try { localStorage.setItem(`project-scheduling-notes-${week.week}`, JSON.stringify(updated)); } catch {}
  };
    ...
    <div style={{ animation: "fadeUp .3s ease" }}>
      ...
      <div className="cd" style={{ padding: 18, marginBottom: 14, borderLeft: `4px solid ${phase?.color || week.color}` }}>
        ...
        <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: hasInfused ? 10 : 0 }}>
          {week.services?.map(s => (
            <span key={s} className="tg" style={{ background: "#14B8A610", color: "#14B8A6", border: "1px solid #14B8A628" }}>
              {s}
            </span>
          ))}
        </div>
        ...
        {["overview", "prompts", "labs", ...(week.project ? ["project"] : []), "resources"].map(t => (
          <button 
            ...
            style={{
              ...
              background: activeTab === t ? "#14B8A6" : "var(--card-bg)",
              color: activeTab === t ? "#042f2e" : "var(--text-muted)",
      ...
      {activeTab === "prompts" && (
        <div style={{ display: "grid", gap: 10 }}>
          <div style={{ padding: 10, background: "#14B8A610", border: "1px solid #14B8A628", borderRadius: 10, fontSize: 12, color: "#14B8A6" }}>
              ...
              <div key={i} className="cd" style={{ padding: 16, animation: `slideIn .3s ease ${i * .03}s both` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  ...
                  <button 
                    ...
                    style={{ padding: "4px 10px", background: copiedId === pid ? "#22c55e" : "#14B8A6", color: copiedId === pid ? "#fff" : "#042f2e", fontSize: 11 }}
                  >
                ...
                {open && (
                  <div style={{ padding: "0 14px 14px", animation: "fadeUp .2s ease" }}>
                    <div style={{ background: "var(--card-bg-alt)", borderRadius: 10, padding: 12, border: "1px solid var(--border)" }}>
                      <div style={{ fontSize: 10, fontWeight: 700, color: "#14B8A6", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>
                        Step-by-Step
                      </div>
                      {lab.steps?.map((s, si) => (
                        <div key={si} style={{ display: "flex", gap: 8, alignItems: "flex-start", marginBottom: si < lab.steps.length - 1 ? 6 : 0 }}>
                          <div style={{ 
                            ...
                            background: "#14B8A616", color: "#14B8A6",
                            display: "flex", alignItems: "center", justifyContent: "center",
        ...
        <div className="cd" style={{ overflow: "hidden" }}>
          ...
          <div style={{ padding: 16 }}>
            ...
            {week.project.prompt && (
              <>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <span style={{ fontSize: 10, fontWeight: 700, color: "#14B8A6", textTransform: "uppercase", letterSpacing: 1 }}>
                    Claude Prompt
                  </span>
                  <button 
                    className="bt" 
                    onClick={() => onCopy(week.project.prompt, week.project.id)}
                    style={{ padding: "4px 10px", background: copiedId === week.project.id ? "#22c55e" : "#14B8A6", color: copiedId === week.project.id ? "#fff" : "#042f2e", fontSize: 11 }}
                  >
                ...
                <pre style={{ 
                  background: "var(--card-bg-alt)", borderRadius: 8, padding: 10, fontSize: 11,
                  color: "var(--text-muted)", lineHeight: 1.5, whiteSpace: "pre-wrap", wordBreak: "break-word",
                  fontFamily: "'JetBrains Mono', monospace", border: "1px solid var(--border)",
          ...
          {(week.resources || []).map((r, i) => (
            ...
            >
              <div style={{ 
                ...
                background: r.type === "video" ? "#DC262610" : "#14B8A610",
                display: "flex", alignItems: "center", justifyContent: "center",
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 18 }}>
        ...
        {week.week < totalWeeks ? (
          <button 
            ...
            style={{ padding: "7px 14px", background: "#14B8A6", color: "#042f2e", fontSize: 12 }}
          >
            Wk {week.week + 1} →
          </button>
        ) : <div />}
