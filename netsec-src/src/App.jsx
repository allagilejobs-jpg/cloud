...
const WEEKS = [
  // ═══════ PHASE 1: NETWORKING FOUNDATIONS (Weeks 1-3) ═══════
  {
    ...
    labs: [
      { id: "lab-1-1", title: "Linux Command Line for Security Engineers", duration: "40 min", difficulty: "Beginner",
        steps: ["Spin up a Linux VM (Ubuntu on VirtualBox or AWS EC2 Free Tier)", "Network recon: ip addr, ip route, ss -tlnp, cat /etc/resolv.conf", "DNS testing: dig google.com, nslookup 8.8.8.8, dig MX gmail.com", "Port scanning yourself: nmap -sT localhost, nmap -sV localhost", "Packet capture: sudo tcpdump -i eth0 -c 20 (capture 20 packets)", "File inspection: find /var/log -name '*.log' -mtime -1 (recent logs)", "Process hunting: ps aux | grep -i ssh, systemctl status sshd", "Permissions: create a test user, restrict SSH to that user only"],
        verify: "You can run network recon, capture packets, and manage services." },
      { id: "lab-1-2", title: "Map the OSI Model with Real Traffic", duration: "30 min", difficulty: "Beginner",
        steps: ["Install Wireshark on your machine", "Start capture on your active interface", "Open a browser → go to http://example.com (HTTP, not HTTPS)", "Stop capture → apply filter: http", "Find the HTTP GET request → right-click → Follow → TCP Stream", "Identify each OSI layer in the packet: Ethernet (L2), IP (L3), TCP (L4), HTTP (L7)", "Apply filter: dns → observe DNS queries before HTTP request", "Apply filter: arp → see ARP resolution at Layer 2"],
        verify: "You can identify L2-L7 data in a real packet capture." },
      { id: "lab-1-3", title: "DNS & DHCP Hands-On", duration: "25 min", difficulty: "Beginner",
        steps: ["Run: dig +trace google.com (see full resolution: root → TLD → auth)", "Run: dig ANY google.com (see all record types)", "Check your DHCP lease: cat /var/lib/dhcp/dhclient.leases (Linux) or ipconfig /all (Windows)", "Wireshark filter: bootp → capture a DHCP DORA exchange (renew lease)", "Test DNS security: dig txt _dmarc.google.com (see email auth record)"],
        verify: "You understand DNS resolution chain and DHCP lease process." }
    ],
    project: null
  },
  {
    ...
    labs: [
      { id: "lab-2-1", title: "Subnetting Speed Drill", duration: "30 min", difficulty: "Intermediate",
        steps: ["Convert these to binary: 192.168.1.1, 10.0.0.255, 172.16.128.0", "Given 10.10.0.0/16, create 8 equal subnets — list each range", "Given 192.168.50.0/24, calculate: /26 subnets → network, broadcast, usable range for each", "Speed test: subnettingpractice.com — complete 20 problems under 10 minutes", "Identify: which of these are private? 8.8.8.8, 10.5.3.2, 172.32.1.1, 192.168.0.1, 169.254.1.1"],
        verify: "You can subnet a /24 into smaller blocks in under 30 seconds." },
      { id: "lab-2-2", title: "Routing Table Analysis", duration: "25 min", difficulty: "Intermediate",
        steps: ["On Linux: ip route show (examine your routing table)", "Identify: default route, connected routes, gateway", "Add a static route: sudo ip route add 10.10.0.0/16 via 192.168.1.1", "Verify: ip route get 10.10.5.5 (trace which route is used)", "Delete the route: sudo ip route del 10.10.0.0/16", "Traceroute: traceroute 8.8.8.8 — count hops and identify each router"],
        verify: "You can read, add, and troubleshoot routes on Linux." }
    ],
  ...
  },
  {
    ...
    labs: [
      { id: "lab-3-1", title: "AWS VPC with DMZ Architecture", duration: "40 min", difficulty: "Intermediate",
        steps: ["Create VPC: 10.0.0.0/16 with 2 public + 2 private subnets", "Launch EC2 in public subnet (web server), one in private subnet (app)", "Configure Security Groups: web → allow HTTP/HTTPS from 0.0.0.0/0, app → allow only from web SG", "Create NAT Gateway for private subnet outbound access", "Test: SSH to public EC2, curl from there to private EC2", "Compare: SG rules to Palo Alto security policies — what's similar/different?"],
        verify: "Web server accessible publicly. App server only reachable from web tier." },
      { id: "lab-3-2", title: "Wireshark: Capture & Analyze Real Traffic", duration: "45 min", difficulty: "Intermediate",
        steps: ["Capture on your interface for 2 minutes while browsing", "Filter: dns → find all DNS queries, check for unusual domains", "Filter: tcp.flags.syn==1 && tcp.flags.ack==0 → see all new connections", "Filter: http.request → see all HTTP requests (unencrypted)", "Filter: tls.handshake.type==1 → see TLS Client Hello with SNI (server names)", "Statistics → Protocol Hierarchy → what % is encrypted?", "Statistics → Conversations → find the top 5 talkers by bytes", "Follow a TCP stream → read the full HTTP conversation", "Export: File → Export Objects → HTTP → save any transferred files"],
        verify: "You can filter, analyze, and extract data from packet captures." }
    ],
