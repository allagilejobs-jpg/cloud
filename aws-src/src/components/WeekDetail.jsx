    ...
    <div style={{ animation: "fadeUp .3s ease" }}>
      ...
      <div className="cd" style={{ padding: 18, marginBottom: 14, borderLeft: `4px solid ${phase?.color || week.color}` }}>
        ...
        <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: hasInfused ? 10 : 0 }}>
          {week.services.map(s => (
            <span key={s} className="tg" style={{ background: "#FF990010", color: "#FF9900", border: "1px solid #FF990028" }}>
              {s}
            </span>
          ))}
        </div>
      ...
      <div style={{ display: "flex", gap: 5, marginBottom: 14, flexWrap: "wrap" }}>
        {["overview", "prompts", "labs", ...(week.project ? ["project"] : []), "resources"].map(t => (
          ...
          >
            {t === "overview" ? "📋 Overview" :
             t === "prompts" ? `💬 Prompts (${week.prompts.length})` :
        ...
        <div className="cd" style={{ padding: 16 }}>
          ...
          {week.topics && week.topics.length > 0 && (
            <div style={{ marginTop: 16 }}>
              <h3 style={{ fontSize: 14, fontWeight: 700, color: "#fff", marginBottom: 8 }}>Topics</h3>
      ...
      {activeTab === "prompts" && (
        <div style={{ display: "grid", gap: 10 }}>
          <div style={{ padding: 10, background: "#FF990010", border: "1px solid #FF990028", borderRadius: 10, fontSize: 12, color: "#FF9900" }}>
              ...
              <div key={i} className="cd" style={{ padding: 16, animation: `slideIn .3s ease ${i * .03}s both` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  ...
                  <button 
                    ...
                    style={{ padding: "4px 10px", background: copiedId === pid ? "#22c55e" : "#FF9900", color: "#0a0e1a", fontSize: 11 }}
                  >
                    {copiedId === pid ? "✓ Copied" : "Copy"}
                  </button>
                </div>
                <pre style={{ 
                  background: "#0a0e1a", borderRadius: 8, padding: 10, fontSize: 11, 
      ...
      {activeTab === "labs" && (
        <div style={{ display: "grid", gap: 10 }}>
          {week.labs.length === 0 && (
            <div className="cd" style={{ padding: 24, textAlign: "center", color: "#64748b" }}>
            ...
            return (
              <div key={lab.id} className="cd" style={{ overflow: "hidden", animation: `slideIn .3s ease ${i * .03}s both` }}>
                ...
                >
                  ...
                  >
                    {done && <span style={{ color: "#0a0e1a", fontSize: 11, fontWeight: 700 }}>✓</span>}
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: done ? "#22c55e" : "#fff" }}>
                      {lab.title || lab.name}
                    </div>
                ...
                {open && (
                  <div style={{ padding: "0 14px 14px", animation: "fadeUp .2s ease" }}>
                    <div style={{ background: "#0a0e1a", borderRadius: 10, padding: 12, border: "1px solid #1e293b" }}>
                      <div style={{ fontSize: 10, fontWeight: 700, color: "#FF9900", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>
                        Step-by-Step
                      </div>
                      {lab.steps.map((s, si) => (
                        <div key={si} style={{ display: "flex", gap: 8, alignItems: "flex-start", marginBottom: si < lab.steps.length - 1 ? 6 : 0 }}>
                          <div style={{ 
                            ...
                            background: "#FF990016", color: "#FF9900",
      ...
      {activeTab === "project" && week.project && (
        <div className="cd" style={{ overflow: "hidden" }}>
          <div style={{ padding: 16, borderBottom: "1px solid #1e293b", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            >
              {progress[week.project.id] && <span style={{ color: "#0a0e1a", fontSize: 11, fontWeight: 700 }}>✓</span>}
            </div>
            <div style={{ flex: 1 }}>
              <h3 style={{ fontSize: 14, fontWeight: 700, color: "#fff" }}>{week.project.title || week.project.name}</h3>
          ...
          <div style={{ padding: 16 }}>
            ...
            {week.project.prompt && (
              <>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <span style={{ fontSize: 10, fontWeight: 700, color: "#FF9900", textTransform: "uppercase", letterSpacing: 1 }}>
                    Claude Prompt
                  </span>
                  <button 
                    className="bt" 
                    onClick={() => onCopy(week.project.prompt, week.project.id)}
                    style={{ padding: "4px 10px", background: copiedId === week.project.id ? "#22c55e" : "#FF9900", color: "#0a0e1a", fontSize: 11 }}
                  >
            ...
            {week.project.dummyData && (
              <div style={{ marginTop: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                  <span style={{ fontSize: 10, fontWeight: 700, color: "#1A73E8", textTransform: "uppercase", letterSpacing: 1 }}>
                    Scenario / Dummy Data
                  </span>
                  <button 
                    className="bt" 
                    onClick={() => onCopy(week.project.dummyData, "d-" + week.project.id)}
                    style={{ padding: "4px 10px", background: copiedId === "d-" + week.project.id ? "#22c55e" : "#1A73E8", color: "#fff", fontSize: 11 }}
                  >
                    {copiedId === "d-" + week.project.id ? "✓" : "Copy"}
                  </button>
                </div>
                <pre style={{ 
                  background: "#0a0e1a", borderRadius: 8, padding: 10, fontSize: 10,
                  color: "#64748b", lineHeight: 1.5, whiteSpace: "pre-wrap",
                  fontFamily: "'JetBrains Mono', monospace", border: "1px solid #1e293b",
          ...
          {(week.resources || []).map((r, i) => (
            ...
            >
              <div style={{ 
                ...
                background: r.type === "video" ? "#DC262610" : "#FF990010",
                display: "flex", alignItems: "center", justifyContent: "center",
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 18 }}>
        ...
        {week.week < totalWeeks ? (
          <button 
            ...
            style={{ padding: "7px 14px", background: "#FF9900", color: "#0a0e1a", fontSize: 12 }}
          >
            Wk {week.week + 1} →
          </button>
        ) : <div />}
